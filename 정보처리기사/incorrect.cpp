...
void chapter1::section3() {
	...
	//d는 .이 없네
	printf("a=%8.2f b=%e c=%4f d=%f\n", 125.23f, 3141.592E-1, 3141.592, 3.483E-2);
...
void chapter3::cp(const std::string& s) {
	cout << s;
	cout << "1. ( 1 )이란 테이블에서 일부 속성들의 종속으로 인해 데이터의 중복이 발생하고, 중복으로 인해\n";
	cout << "테이블 조작 시 문제가 발생하는 현상을 말한다.\n";
	cout << "( 2 ) : 삽입할 때 의도와 원하지 않는 값들로 인해 삽입할 수 없게 되는 현상\n";
	cout << "( 3 ) : 한 튜플을 삭제할 때 의도와는 상관없는 값들도 함께 연쇄 삭제되는 현상\n";
	cout << "( 4 ) : 튜플의 값을 갱신할 때 일부 튜플의 정보만 갱신되어 정보에 불일치성이 생기는 현상\n";
	cout << "2. ( 1 )란 테이블의 속성들이 상호 종속적인 관계를 ( 2 )하는 과정이다.\n";
	cout << "중복을 제거하여 삽입, 삭제, 갱신 이상이 발생할 가능성을 줄일 목적으로 실행한다.\n";
	cout << "( 2 )는 테이블의 일부 속성들만 추출한 것을 다시 JOIN 했을 때 손실 없이 원래 테이블로 복구되는 것을 말한다.\n";
	cout << "3. 논리적 설계(데이터 모델링)를 물리적 설계로 변환하는 규칙의 빈 칸을 채우시오.\n";
	cout << "( 1 ) -> 테이블\t속성 -> ( 2 )\t( 3 ) -> 기본키\n";
	cout << "외부 식별자 -> ( 4 )\t ( 5 ) -> 관계\n";
	cout << "4. 반정규화에 대해서 간단하게 서술하시오.\n";
	cout << "5. 반정규화의 ( 1 ) 은 두 개의 테이블이 ( 2 )되는 경우가 많아 하나의 테이블로 합쳐 사용하는 것이다.\n";
	cout << "( 3 )은 테이블을 수직 또는 수평으로 분할하는 것으로, 레코드별 사용 빈도 차이가 큰 경우 레코드를 기준으로 수평 분할,\n";
	cout << "하나의 테이블에 속성이 너무 많은 경우, 자주 조회, 갱신되는 속성이나 보안을 적용해야 하는 속성을 기준으로 수직 분할한다.\n";
	cout << "( 4 ) 테이블 추가는 여러 테이블에서 데이터를 추출해야 하는 경우 집계 테이블 or 특정 부분만을 포함하는 테이블을 추가하는 것이다.\n";
	cout << "( 4 ) 속성 추가는 ( 2 )해서 데이터를 처리할 때, 데이터를 조회하는 경로를 단축하는 것이다.\n";
	cout << "6. 인덱스에 대해서 간단하게 서술하시오.\n";
	cout << "7. 인덱스의 종류로는 다음과 같은 것이 있다.\n";
	cout << "( 1 ) : 인덱스를 저장하는 블록들이 b - 트리 구조를 이루고 있어서 루트 노드에서 하위 노드로 키 값의 크기를 비교해나가며 검색\n";
	cout << "( 2 ) : 인덱스 컬럼의 데이터를 Bit 값인 0 또는 1로 변환하여 인덱스 키로 사용한다. 로우의 주소를 제공\n";
	cout << "( 3 ) : 특정 함수나 수식을 컬럼에 적용하여 산출된 값을 사용함. \n";
	cout << "( 4 ) : 개발자가 필요한 인덱스를 직접 만들어 사용하는 확장형 인덱스\n";
	cout << "8. ( 1 )는 데이터 저장 시 데이터 액세스 효율을 향상시키기 위해 동일한 성격의 데이터를 동일한 데이터 블록에 저장하는 물리적 저장법이다.\n";
	cout << "분포도가 ( 2 ) 테이블, 입력, 수정, 삭제가 ( 3 ) 테이블, 자주 ( 4 )되는 테이블, ORDER BY, GROUP BY, UNION 등이 ( 5 ) 테이블에 적용한다.\n";
	cout << "9. 파티션에 대해 간단하게 서술하시오. (장, 단점 포함)\n";
	cout << "10 파티션의 종류는 다음과 같다.\n";
	cout << "( 1 ) : 지정한 열의 값을 기준으로 분할\n";
	cout << "( 2 ) : 해시 함수를 적용한 결과 값에 따라 데이터 분할. 데이터가 집중되는 범위 분할의 단점 보완, but 데이터가 어딨는지 모름\n";
	cout << "( 3 ) : 범위 분할 한 다음 해시 함수를 적용. 범위 분할한 파티션이 너무 클 때 사용.\n";
	kh();
	cout << "  1. ( 1 ) : 이상,  ( 2 ) : 삽입 이상,  ( 3 ) : 삭제 이상,  ( 4 ) : 갱신 이상\n\n";
	cout << "  2. ( 1 ) : 정규화,  ( 2 ) : 무손실 분해\n\n";
	cout << "  3. ( 1 ) : Entity,  ( 2 ) : Column,  ( 3 ) : 주 식별자,  ( 4 ) : 외래키,  ( 5 ) : 관계\n\n";
	cout << "  4. 시스템의 성능 향상 및 개발과 관리의 편의성을 위해 정규화된 데이터를 통합, 중복, 분할하는 과정이다. 데이터 일관성이 떨어질 수 있다.\n\n";
	cout << "  5. ( 1 ) : 테이블 통합,  ( 2 ) 조인,  ( 3 ) 테이블 분할,  ( 4 ) : 중복\n\n";
	cout << "  6. 데이터 테이블을 빠르게 접근하기 위해 키, 포인터 쌍으로 구성되는 데이터 구조다.\n";
	cout << "  인덱스가 없으면 TABLE SCAN이 발생해 액세스가 매우 느리며, 인덱스를 통해 파일 레코드에 대한 액세스를 빠르게 할 수 있다.\n\n";
	cout << "  7. ( 1 ) : 트리 기반 인덱스,  ( 2 ) : 비트맵 기반 인덱스,  ( 3 ) : 함수 기반 인덱스,  ( 4 ) : 도메인 인덱스\n\n";
	cout << "  8. ( 1 ) : 클러스터,  ( 2 ) : 넓은,  ( 3 ) : 적은,  ( 4 ) : 조인,  ( 5 ) 빈번한\n\n";
	cout << "  9. 대용량 테이블이나 인덱스를 작은 논리적 단위인 파티션으로 나누는 것이다. 데이터 접근 액세스 범위를 줄이고,\n";
	cout << "  분산 저장을 함.(백업 및 복구 향상, 손상 최소화, 입 출력 분산) 다만, 테이블간 조인에 대한 비용이 증가한다.\n\n";
	cout << "  10. ( 1 ) : 범위 분할,  ( 2 ) : 해시 분할,  ( 3 ) : 조합 분할\n\n";

	cout << "11. 데이터 모델의 개념을 간략히 서술하시오\n";
	cout << "12. 데이터 모델에 표시할 요소에 대한 설명이다.\n";
	cout << "( 1 ) : 데이터베이스에 저장된 실제 데이터를 처리하는 작업에 대한 명세로, 조작하는 기본 도구\n";
	cout << "( 2 ) : 데이터베이스에 저장될 수 있는 실제 데이터의 논리적 제약 조건\n";
	cout << "( 3 ) : 논리적으로 표현된 개체 타입들 간의 관계로서 데이터 구조 및 정적 성질\n";
	cout << "13. (   )이란 R 테이블에서 A와 C를 각각 R의 속성 집합에 대한 부분 집합이라고 할 경우, 속성 A의 값 각각에 대해 시간에 관계 없이\n";
	cout << "항상 속성 C의 값이 오직 하나만 연관되어야 하는 제약 조건이다.\n";
	kh();
	cout << "  11. 현실 세계의 물체나 개념을 컴퓨터로 표현하기 위해서 개체, 개체들간의 관계 등을 단순화, 추상화하여 표현하는 것을 말한다.\n\n";
	cout << "  12. ( 1 ) : 연산,  ( 2 ) : 제약 조건,  ( 3 ) : 구조\n\n";
	cout << "  13. 함수적 종속\n\n";
}
...
void chapter4::cp(const std::string& s) {
	cout << s;
	cout << "1. 통합 구현의 5가지 구성 요소\n";
	cout << "2. 송, 수신 시스템 직접 연계 방식의 장, 단점\n";
	cout << "3. 송, 수신 시스템 간접 연계 방식의 장, 단점\n";
	cout << "4. 직접 연계 방식의 ( 1 ) : DB에서 제공하는 DB Link 객체를 이용하는 방식.\n";
	cout << "( 2 ) : 수신 시스템의 WAS에서 송신 시스템의 DB로 연결\n";
	cout << "( 3 ) : 데이터를 송신 시스템의 DB에서 읽어와 제공하는 프로그래밍 인터페이스\n";
	cout << "( 4 ) : Java에서 Db에 접근하여 데이터를 수정할 수 있도록 도와주는 방식.\n";
	cout << "간접 연계 방식의 ( 5 ) : 앱간 연계, 데이터 변환, 웹 서비스 지원 등 표준 기반의 인터페이스를 제공. 서비스 연동에 중심점\n\n";
	cout << "( 6 ) : 서버는 통신을 위한 소켓을 생성, 포트 할당. 클라이언트와 연결\n"; 
	cout << "( 7 ) : 웹 서비스에서 SOAP, WSDL 등 프로토콜을 이용하여 연계\n";
	cout << "5. 웹 서비스의 세 가지 구성 요소는?\n";
	cout << "( 1 ) : HTTP, HTTPS 등을 활용하여 XML 기반의 메시지를 네트워크 상에서 교환하는 프로토콜\n";
	cout << "( 2 ) : 웹 서비스명, 서비스 제공 위치, 프로토콜 등 웹 서비스에 대한 상세 정보(통신 규약)를 XML 형식으로 구현하는 언어\n";
	cout << "( 3 ) : ( 2 )를 등록하여 서비스, 서비스 제공자를 검색하고 접근하는데 사용.\n";
	kh();
	cout << "  1. 송신 시스템, 수신 시스템, 중계 시스템, 연계 데이터, 네트워크\n\n";
	cout << "  2. 구현이 단순하고 용이하고, 비용이 저렴하고, 데이터 처리 성능이 뛰어나지만, 결합도가 높아 시스템 변경시 오류 발생.\n";
	cout << "  암호화, 비즈니스 로직 적용하기가 어려움.\n\n";
	cout << "  3. 중간에 매개체를 두어 송, 수신 시스템 연결. 서로 다른 네트워크, 프로토콜, 인터페이스 상호 연동 쉬움. 보안 보장, 비즈니스 로직 적용\n";
	cout << "  다만, 아키텍처가 복잡하고 성능이 저하되고, 개발 기간이 좀 길다.\n\n";
	cout << "  4. ( 1 ) : DB LINK,  ( 2 ) : DB Connection,  ( 3 ) : API,  ( 4 ) : JDBC,\n";
	cout << "  ( 5 ) : ESB,  ( 6 ) : Socket,  ( 7 ) : Web Service\n\n";
	cout << "  5. ( 1 ) : SOAP,  ( 2 ) : WDSL,  ( 3 ) : UDDI\n\n";

	cout << "6. 구축된 연계 시스템과 연계 시스템의 구성 요소가 정상적으로 동작하는지 확인하는 연계 테스트를 수행했을 때, 수행 결과를 검증하는 방법 세 가지는?\n";
	cout << "7. 연계 메커니즘에서 송신 시스템, 수신 시스템의 주요 기능 및 역할을 순서별로 서술하시오\n";
	cout << "8. 연계 데이터 보안 적용시 암, 복호화 적용 대상은 중요도에 따라 선정한다. 중요도는 시스템에 따라 다르지만, 공통적으로는\n";
	cout << "주민번호, 운전면허번호, 신용카드번호 등 (   )에 근거한 개인정보가 있다.\n";
	cout << "9. WSDL(v2.0)의 각 요소에 대한 설명이다.\n";
	cout << "( 1 ) : Data Type을 정의한다.\n";
	cout << "( 2 ) : ( 3 )을 통해 웹 서비스의 인터페이스를 정의한다.\n";
	cout << "( 3 ) : 메소드의 요청 및 응답 메시지를 정의한다.\n";
	cout << "( 4 ) : 웹 서비스를 이용할 때 사용할 통신 방법을 정의한다.\n";
	cout << "( 5 ) : port/( 6 )을 통해 웹 서비스를 정의한다.\n";
	cout << "( 6 ) : 웹 서비스의 URL과 ( 4 )를 연결한다.\n";
	kh();
	cout << "  6. 서버 모니터링 현황 확인, 파일 생성 위치의 파일 생성 확인 및 크기 확인, 시스템 로그 확인\n\n";
	cout << "  7. 연계 데이터 생성 -> 코드 맵핑 및 데이터 변환 -> 인터페이스 테이블 또는 파일 생성 -> 연계 서버 전송 ->\n";
	cout << "  연계 서버 수신 -> 인터페이스 테이블 또는 파일 생성 -> 코드 맵핑 및 데이터 변환 -> 연계 데이터 DB에 반영\n\n";
	cout << "  8. 개인정보보호법\n\n";
	cout << "  9. ( 1 ) : types,  ( 2 ) : interface,  ( 3 ) : operation,  ( 4 ) : binding,\n";
	cout << "  ( 5 ) : service,  ( 6 ) : endpoint\n\n";
}
